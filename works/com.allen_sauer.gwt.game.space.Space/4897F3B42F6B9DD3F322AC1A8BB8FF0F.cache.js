...
function debug(text){
  ...
  if (debugTextArea === null) {
    ...
    $setHorizontalAlignment(debugTable.cellFormatter, 0, 0, ($clinit_69() , ALIGN_CENTER));
    $run(timer_0);
...
function round_int(x){
  if (x > ($clinit_118() , MAX_VALUE))
    return $clinit_118() , MAX_VALUE;
  if (x < ($clinit_118() , MIN_VALUE))
    return $clinit_118() , MIN_VALUE;
  return x >= 0?Math.floor(x):Math.ceil(x);
}

function round_long(x){
  if (x > ($clinit_119() , MAX_VALUE_0))
    return $clinit_119() , MAX_VALUE_0;
  if (x < ($clinit_119() , MIN_VALUE_0))
    return $clinit_119() , MIN_VALUE_0;
  return x >= 0?Math.floor(x):Math.ceil(x);
}
...
function onWindowClosed_0(){
  var gwt, it;
  for (it = $iterator_2($values(($clinit_87() , rootPanels))); $hasNext_2(it);) {
...
function $HashMap$EntrySetIterator(this$static, this$0){
  ...
  if (this$static.this$0.nullSlot !== ($clinit_145() , UNDEFINED)) {
